#!/usr/bin/perl -w
# Program name:		format_DESeq2_result_strict.pl
...
if (($t[6]<0.05)&&($t[2]>1)) {print Profile "$_,\"Up\",\"$opts{b}\",\"$opts{c} vs $opts{b}\"\n";}



if (($t[6]<0.05)&&($t[2]<-1)) {print Profile "$_,\"Down\",\"$opts{b}\",\"$opts{c} vs $opts{b}\"\n";}
